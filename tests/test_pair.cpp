...
TEST(pair, set_get_pair_vector) {
    auto V = spla::Vector::make(2, spla::PAIR);
    V->set_pair(0, spla::T_PAIR(1.0f, 1));
    V->set_pair(1, spla::T_PAIR(2.0f, 2));
    spla::T_PAIR pair;
    V->get_pair(0, pair);
    EXPECT_EQ(pair.vertex, 1);
    EXPECT_EQ(pair.weight, 1.0f);
    V->get_pair(1, pair);
    EXPECT_EQ(pair.vertex, 2);
    EXPECT_EQ(pair.weight, 2.0f);
}
TEST(pair, set_get_pair_scalar) {
    auto         V    = spla::Scalar::make(spla::PAIR);
    spla::T_PAIR pair = spla::T_PAIR(1.0f, 1);
    V->set_pair(pair);
    V->get_pair(pair);
    EXPECT_EQ(pair.vertex, 1);
    EXPECT_EQ(pair.weight, 1.0f);
...
TEST(pair, mxv_pair) {
    ...
    spla::T_PAIR init_val(1e9f, -1);
    init_inf->set_pair(init_val);
    spla::exec_mxv_masked(edge, mask, S, parent, spla::MUL_PAIR, spla::MIN_PAIR,
                          spla::ALWAYS_PAIR, init_inf);

    spla::T_PAIR expected[] = {spla::T_PAIR(4.0f, 4), spla::T_PAIR(7.0f, 0),
                               spla::T_PAIR(5.0f, 3), spla::T_PAIR(5.0f, 2),
                               spla::T_PAIR(4.0f, 0), spla::T_PAIR(6.0f, 4),
                               spla::T_PAIR(8.0f, 3)};
...
TEST(pair, extract_row) {
    ...
    spla::T_PAIR expected[] = {spla::T_PAIR(4.0f, 4), spla::T_PAIR(),
                               spla::T_PAIR(7.0f, 0)};
    for (int32_t i = 0; i < n; i++) {
        spla::T_PAIR p;
        row1->get_pair(i, p);
...
TEST(pair, m_assign_bslct) {
    int32_t n = 3;
    auto    S = spla::Matrix::make(n, n, spla::PAIR);
    S->set_pair(0, 1, spla::T_PAIR(7.0f, 1));
    S->set_pair(1, 0, spla::T_PAIR(4.0f, 4));
    S->set_pair(1, 2, spla::T_PAIR(7.0f, 0));
    S->set_pair(2, 0, spla::T_PAIR(3.0f, 8));
    S->set_pair(2, 1, spla::T_PAIR(6.0f, 9));

    auto mask = spla::Vector::make(3, spla::PAIR);
    mask->set_pair(0, spla::T_PAIR(0.0f, 4));
    mask->set_pair(1, spla::T_PAIR(0.0f, 4));
    mask->set_pair(2, spla::T_PAIR(0.0f, 5));

    auto         init_scalar = spla::Scalar::make(spla::PAIR);
    spla::T_PAIR init_val;
    init_scalar->set_pair(init_val);

    spla::exec_m_assign_bslct_masked(S, mask, init_scalar, spla::SECOND_PAIR, spla::EQVERTEX_PAIR);

    spla::T_PAIR expected[] = {spla::T_PAIR(), spla::T_PAIR(), spla::T_PAIR(),
                               spla::T_PAIR(), spla::T_PAIR(), spla::T_PAIR(7.0f, 0),
                               spla::T_PAIR(3.0f, 8), spla::T_PAIR(6.0f, 9), spla::T_PAIR()};
